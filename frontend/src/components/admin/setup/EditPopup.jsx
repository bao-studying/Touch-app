...
export default function EditPopup({ title, onClose, children, footer }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-espresso-950/50 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full sm:max-w-md max-h-[88vh] rounded-t-3xl sm:rounded-3xl bg-cream-50 shadow-xl flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-espresso-900/8 shrink-0">
          ...
          <button onClick={onClose} className="text-espresso-700/50 p-1" aria-label="Đóng">
            <X size={20} />
          </button>
        </div>
